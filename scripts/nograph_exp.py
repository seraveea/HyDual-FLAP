...
from utils import format_date, get_trading_days, Bert_embeder, x_month_ago, argument_keyword, BGE_embeder, get_first_and_last_trading_days, format_symbol
from utils import ll3_instance, ds_instance, fingpt_instance, chatqa_instance, selfrag_instance
...
def main(args):
    ...
    dataset = dataset.map(format_symbol)
    ...
    elif args.backbone == 'chatqa':
        llm = chatqa_instance(args)
    else:
    ...
    if args.backbone == 'fingpt':
        rag_agent = FinGPT_forecaster(llm, ts, summary, maper, embeder, style=args.style, language=args.language)
    else:
        rag_agent = vanilla_RAG(llm, ts, summary, maper, embeder, args.preload_doc_path, style=args.style, language=args.language)  
